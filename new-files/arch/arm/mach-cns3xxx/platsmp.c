...
/* 
 * I fully acknowledge the irony in the comment below. 
 * I admit that I don't really know what I'm doing here. I'm just
 * getting things to "work". This is one reason why this code
 * will not / should not / must not be included in mainline 
 * linux.
 *
 * XXX cns3xxx_pen_release is cargo culted code - DO NOT COPY XXX 
 */
volatile int cns3xxx_pen_release = -1;
...
/*
 * Write cns3xx_pen_release in a way that is guaranteed to be visible to all
 * observers, irrespective of whether they're taking part in coherency
 * or not.  This is necessary for the hotplug code to work reliably.
 */
static void cns3xxx_write_pen_release(int val)
{
	cns3xxx_pen_release = val;
	smp_wmb();
	sync_cache_w(&cns3xxx_pen_release);
//	__cpuc_flush_dcache_area((void *)&cns3xxx_pen_release, sizeof(cns3xxx_pen_release));
//	outer_clean_range(__pa(&cns3xxx_pen_release), __pa(&cns3xxx_pen_release + 1));
}
...
static int cns3xxx_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	 ...
	 * that it has been released by resetting cns3xxx_pen_release.
	 *
	 * Note that "cns3xxx_pen_release" is the hardware CPU ID, whereas
	...
	cns3xxx_write_pen_release(cpu);
	smp_rmb();
	timeout = jiffies + (1 * HZ); 
	while (time_before(jiffies, timeout)) {
		smp_rmb();
		if (cns3xxx_pen_release == -1)
			break;
	...
	return cns3xxx_pen_release != -1 ? -ENOSYS : 0;
}
